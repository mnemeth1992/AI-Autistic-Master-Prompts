...
# Core & Module imports (supports both flat root and nested 'app/' folder structures)
try:
...
def inject_custom_css():
    ...
    candidates = [
        ...
        os.path.join(current_dir, "app", "assets", "style.css"),
    ]
    css_loaded = False
    for p in candidates:
        if os.path.exists(p):
...
def render_settings_module(km):
    ...
    if sa_existing:
        ...
        st.info("💡 Helyi módban automatikusan a laptop meghajtóját használja. Streamlit Cloudhoz illeszd be a Google Service Account JSON kulcsodat a Secrets-be vagy ide:")
