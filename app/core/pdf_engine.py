"""
ReportLab PDF Interior Compilation Engine for KDP Books
======================================================
Generates print-ready Amazon KDP interior PDF documents:
  - Coloring books (8.5x11, 8.5x8.5, 8x10, 6x9):
      * Page 1: Title Page & Color/Marker Test Swatches
      * Companion Pages (Even): Scripture Verse + Palette + Reflection
      * Main Coloring Pages (Odd): Scaled High-Res PNG Image with safe margins
      * Bleed Protection Pages (Even): Clean blank pages to prevent marker bleed
  - Illustrated Storybooks (8.5x8.5, 8.5x11, etc.):
      * Title Page & Dedication
      * Formatted Chapter Layout with Top Illustrations & Bottom Narrative
"""
...
try:
    from core.kdp_math import parse_trim_size
except (ModuleNotFoundError, ImportError):
    from app.core.kdp_math import parse_trim_size
...
def get_kdp_page_metrics(trim_size: str = "8.5x11", margin_in: float = 0.5) -> Tuple[float, float, float, float, float]:
    ...
    for any Amazon KDP trim size and custom margin.
    """
...
def get_color_for_name(name: str) -> Color:
    """Matches a color name string to a ReportLab Color object."""
...
def draw_title_and_tester_page(
    ...
    # "This Book Belongs To" decorative box
...
def draw_coloring_image_page(
    ...
    if pil_img is not None:
        ...
        c.drawCentredString(pw / 2.0, ph / 2.0, "⚠️ Nincs elérhető kép ehhez az oldalhoz.")
...
def build_illustrated_book_pdf(
    ...
    """Compiles an illustrated storybook with narrative text and color illustrations into KDP PDF."""
    if not REPORTLAB_AVAILABLE:
    ...
    try:
        ...
        c.setFillColor(HexColor("#334155"))
        c.setFont("Helvetica-Bold", 12)
        c.drawCentredString(pw / 2.0, margin + 1.4 * 72, "Written & Illustrated by")
